  ...
  return (
    <motion.div
      animate={{
        ...
        backgroundColor: visible ? 'rgba(23, 23, 23, 0.8)' : 'rgba(0, 0, 0, 0)',
        boxShadow: visible
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-base font-medium text-white/70 transition duration-200 hover:text-white lg:flex lg:space-x-2',
        className
      )}
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-white/70 dark:text-neutral-300"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
...
export const MobileNavMenu = ({
  ...
  void _onClose // Intentionally unused parameter
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
